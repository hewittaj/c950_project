...
only_truck_two = [3, 18, 35, 37, 38, 27, 28, 32, 33, 36, 40]  # Packages that can only be on truck two
delayed = [6, 25, 31]  # Packages that are delayed and won't arrive until 9:05 go to truck two
...
while truck1:
    ...
    if len(truck1) == 0:
        truck1.append(['0', '4001 South 700 East'])  # Set truck to return to hub
        distance_info = d.get_greedy_route(truck1, greedy_route_truck1)  # Get distance info for the next truck
        truck1_distance_info.append(distance_info)  # Append to our list for our later time calculations
        greedy_route_truck1.append(['0', '4001 South 700 East'])  # Add package to greedy_route
